...
function forestStoryHtml(nm){
  return fetchForestStory(nm).then(function(o){
    ...
    var html='<div style="border-top:1px solid #E6E6E6;padding-top:24px;margin-top:8px"><p style="font-size:11px;font-weight:600;letter-spacing:1.5px;color:#121212;margin:0 0 12px">숲이야기 · 산림청(산림문화·휴양정보)</p>';
    if(story)html+='<p style="color:#121212;font-size:14px;line-height:1.9;margin:0 0 14px;white-space:pre-line">'+esc(story)+'</p>';
...
function nongsaroSection(title,rows){
  ...
  var html='<div style="border-top:1px solid #E6E6E6;padding-top:24px;margin-top:8px">'
    +'<p style="font-size:11px;font-weight:600;letter-spacing:1.5px;color:#121212;margin:0 0 12px">'+esc(title)+'</p>';
  html+='<table style="width:100%;border-collapse:collapse">';
  rows.forEach(function(r){
    html+='<tr style="border-bottom:1px solid #E6E6E6"><td style="padding:12px 0;color:#ABABAB;width:28%;font-size:11px;letter-spacing:1px;vertical-align:top;font-weight:600">'+esc(r[0])+'</td><td style="padding:12px 0;color:#121212;font-size:14px;line-height:1.7">'+esc(r[1])+'</td></tr>';
  });
  html+='</table></div>';
  return html;
}
...
function bookFtcStoryHtml(r){
  ...
  var html='<div style="border-top:1px solid #E6E6E6;padding-top:24px;margin-top:8px">'
    +'<p style="font-size:11px;font-weight:600;letter-spacing:1.5px;color:#121212;margin:0 0 12px">이름의 유래 · 국립수목원 발간자료</p>';
  if(r.originKor)html+='<p style="color:#121212;font-size:14px;line-height:1.85;margin:0 0 14px"><b style="font-weight:600">국명</b> — '+esc(r.originKor)+'</p>';
...
function bookForest300Html(r){
  ...
  var html=nongsaroSection('국립수목원 발간자료 · 숲정원을 위한 식물 300종',rows);
...
function envBarHtml(label,options,active){
  return '<div style="margin-bottom:16px">'
    +'<p style="font-size:10px;letter-spacing:1.5px;color:#ABABAB;margin:0 0 6px">'+esc(label)+'</p>'
...
function curatedLandscapeHtml(p){
  ...
  var tagsHtml=badges.map(function(t){return '<span style="display:inline-block;border:1px solid #DCDCDC;padding:3px 10px;font-size:11px;color:#787878;margin:0 6px 6px 0;letter-spacing:.3px">#'+esc(t)+'</span>';}).join('');
  return ''
...
function renderCompareBar(){
  ...
  thumbs.innerHTML=Object.keys(pCompareSet).map(function(uid){
    ...
    return '<div style="position:relative;flex-shrink:0" title="'+esc(rec.it.nm)+'">'
      +(src?'<img src="'+src+'" style="width:40px;height:40px;object-fit:cover;border-radius:4px;display:block">':'<div style="width:40px;height:40px;border-radius:4px;background:#333;display:flex;align-items:center;justify-content:center;font-size:16px">&#127807;</div>')
...
function renderPage(){
  ...
  next.forEach(function(it){
    ...
    d.innerHTML='<div class="pc-img">&#127807;</div><button type="button" class="pc-cmpbtn">비교</button><div class="pc-body"><div class="pc-core">'+coreHtml(it)+'</div></div>';
...
function rowsTable(rows){
  if(!rows.length)return '<p style="color:#ABABAB;text-align:center;padding:20px 0">상세 정보가 없습니다.</p>';
  var html='<table style="width:100%;border-collapse:collapse">';
  rows.forEach(function(r){
    html+='<tr style="border-bottom:1px solid #E6E6E6"><td style="padding:14px 0;color:#ABABAB;width:32%;font-size:11px;letter-spacing:1px;vertical-align:top;font-weight:600">'+esc(r[0].toUpperCase())+'</td><td style="padding:14px 0;color:#121212;font-size:14px;line-height:1.7">'+esc(r[1])+'</td></tr>';
  });
  html+='</table>';
  return html;
}
...
/* "학술 정보도 개요에 포함" 요청에 따라 학술정보 탭을 없애고 개요 탭 끝에
   이어붙인다. 개요는 각 분기(도감/표본/특산 등)에서 이미 동기적으로 먼저
   채워지고, 학술 조각은 그보다 늦게 비동기로 도착하므로 덮어쓰지 않고
   이어붙인다. */
function pdAppend(key,html){if(!html)return;var p=pdPane(key);if(p)p.innerHTML+=html;}
function pdSetLoading(key){pdSet(key,'<p style="color:#ABABAB;text-align:center;padding:40px 0">정보 불러오는 중...</p>');}
function pdSetEmpty(key,msg){pdSet(key,'<p style="color:#ABABAB;text-align:center;padding:20px 0;line-height:1.7">'+esc(msg||'정보가 없습니다.')+'</p>');}
function pdSetError(key,msg){pdSet(key,'<p style="color:#DC2B2B;text-align:center;padding:20px 0">'+esc(msg)+'</p>');}
function renderPdTabBar(){
  var bar=document.getElementById('pdtabbar');
  bar.innerHTML=PD_TABS.map(function(t,i){return '<div class="pdtab'+(i===0?' active':'')+'" data-pdtab="'+t[0]+'">'+esc(t[1])+'</div>';}).join('');
  Array.prototype.forEach.call(bar.querySelectorAll('.pdtab'),function(el){
    el.onclick=function(){pSwitchTab(el.getAttribute('data-pdtab'));};
  });
}
window.pSwitchTab=function(key){
  PD_TABS.forEach(function(t){var pane=pdPane(t[0]);if(pane)pane.style.display=(t[0]===key)?'block':'none';});
  var bar=document.getElementById('pdtabbar');
  Array.prototype.forEach.call(bar.querySelectorAll('.pdtab'),function(el){
